...
`daily-workbench.yml` 和 `weekly-gc.yml` 改用官方 `openai/codex-action@v1`,通过仓库 Secret `OPENAI_API_KEY` 调用。北京时间 05:00 日更保持不变;每周 GC 按用户 2026-07-16 的要求改为周一 04:00。安静周跳过、lint、索引重建和直接推 `main` 的行为保持不变。
