...
把本仓库 `templates/repo-pointer.md` 的内容原样写成该仓库根目录的 `CLAUDE.md` 和 `GEMINI.md`(两个文件、内容相同;若已存在 CLAUDE.md,则把指针内容追加进去而不是覆盖),提交并推送;不能直接推默认分支就开 PR。装完当场生效,不需要用户手动复制任何东西。
